...
def get_players_dict(items, players):
    ...
    for p in players:
        ...
        p_wins = c.get_wins(filter_by_player(p, main_stats))
        p_losses = c.get_losses(filter_by_player(p, main_stats))
        p_total = p_wins + p_losses
        if p_total < 20:
            continue
        p_ratio = c.p(p_wins, p_losses)
        easy_ratio, null_ratio, sd_ratio, dd_ratio =\
            c.p(len(filter_by_player(p, list_easy)), p_total),\
            c.p(len(filter_by_player(p, list_null)), p_total),\
            c.p(len(filter_by_player(p, list_sd)), p_total),\
            c.p(len(filter_by_player(p, list_dd)), p_total)
...
def filter_by_player(player, stats):
    return [row for row in stats if player in row['playerNames']]
...
def get_overall_wr(items, players):
    ...
    # print(stats_3)
    for p in players:
        p_wins = c.get_wins(filter_by_player(p, stats_3))
        p_losses = c.get_losses(filter_by_player(p, stats_3))
        p_total = p_wins + p_losses
        if p_total <= 100:
            continue
        p_ratio = c.p(p_wins, p_losses)
...
def form_totals_dict(item, totals_dict, row):
    ...
    if row['score'] == c.get_max_score(row['options']['variantName']):
