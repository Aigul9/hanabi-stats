...
def get_players_dict(username, players):
    ...
    for p in players:
        ...
        easy_ratio, null_ratio, sd_ratio, dd_ratio =\
            c.p(len(get_filtered_by_player(p, list_easy)), ms),\
            c.p(len(get_filtered_by_player(p, list_null)), ms),\
            c.p(len(get_filtered_by_player(p, list_sd)), ms),\
            c.p(len(get_filtered_by_player(p, list_dd)), ms)
