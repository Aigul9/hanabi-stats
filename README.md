...
- ```output```: ```highest_wr_bga.tsv``` contains statistics for no variant, 6 suits and rainbow. ```highest_wr_not_bga.tsv``` excludes aforementioned variants and white reversed, black, dark rainbow which are typically played in speedruns. ```output/filtered_by_players``` contains list of teammates sorted by win/loss ratio with at least 20  games in total, excluding speedrun variants.
